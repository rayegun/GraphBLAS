...
)
{
    ...
    GB_void *p = GB_malloc_memory (4, GrB_INT64, &nbytes) ;
    CHECK (p != NULL) ;
    GB_FREE_WORK (&p, nbytes) ;
    CHECK (p == NULL) ;
    p = GB_calloc_memory (4, GrB_INT64, &nbytes, NULL) ;
    CHECK (p != NULL) ;
    bool ok = true ;
    p = GB_realloc_memory (6, GrB_INT64, p, &nbytes, &ok, NULL) ;
    CHECK (p != NULL) ;
    CHECK (ok) ;
    GB_FREE_WORK (&p, nbytes) ;
    CHECK (p == NULL) ;
