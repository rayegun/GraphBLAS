...
//          break ;
    ...
    switch (format)
    {
        case GrB_CSR_FORMAT : 
        case GrB_CSC_FORMAT : 
            Ap_copy = GB_MALLOC (GrB_Index, plen, GrB_UINT64, &Ap_size) ;
            Ai_copy = GB_MALLOC (GrB_Index, nvals, GrB_UINT64, &Ai_size) ;
            Ax_copy = GB_MALLOC (GB_void, nvals*typesize, type,   &Ax_size) ; // x:OK
            ok = (Ap_copy != NULL && Ai_copy != NULL && Ax_copy != NULL) ;
            break ;

//      case GrB_DENSE_ROW_FORMAT :
//      case GrB_DENSE_COL_FORMAT :
//          Ax_copy = GB_MALLOC (ptrtype, nvals*ptrtypesize, type,   &Ax_size) ; // x:OK
//          ok = (Ax_copy != NULL) ;
//          break ;
