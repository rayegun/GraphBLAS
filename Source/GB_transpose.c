...
)
{
    ...
    else if (avdim == 1)
    {
        ...
        T->p = GB_MALLOC (int64_t, anz+1, GrB_INT64, &(T->p_size)) ;
        T->i = GB_MALLOC (int64_t, anz  , GrB_INT64, &(T->i_size)) ;
    ...
    else if (avlen == 1)
    {
        ...
        T->p = GB_CALLOC (int64_t, 2, GrB_INT64, &(T->p_size)) ;
        if (!A_is_hyper)
        { 
    ...
    { 
        ...
        { 
            ...
            { 
                ...
                Cx_new = GB_CALLOC (GB_void, anz_held*csize, ctype, &Cx_size) ; // x:OK
            }
            else
            { 
                // malloc is fine; all C->x will be written
                Cx_new = GB_MALLOC (GB_void, anz_held*csize, ctype, &Cx_size) ; // x:OK
            }
            if (Cx_new == NULL)
            { 
                // out of memory
