...
)
{
    ...
    size_t header_size ;
    GB_Pending Pending = GB_MALLOC_WORK (1, struct GB_Pending_struct, &header_size) ;
    if (Pending == NULL)
    { 
        // out of memory
        return (false) ;
    ...
    Pending->i = GB_MALLOC_WORK (nmax, int64_t, &(Pending->i_size)) ;
    Pending->j = NULL ;
    if (is_matrix)
    { 
        Pending->j = GB_MALLOC_WORK (nmax, int64_t,  &(Pending->j_size)) ;
    }
    Pending->x = NULL ;
    if (!iso)
    { 
        Pending->x = GB_MALLOC_WORK ( nmax * Pending->size, GB_void,  // x:OK
            &(Pending->x_size)) ;
    }
