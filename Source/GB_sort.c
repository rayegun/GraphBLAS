...
)
{
    ...
    else
    {
        ...
        P->i = GB_MALLOC (int64_t, cnz, GrB_INT64, &(P->i_size)) ;
        if (P->i == NULL)
        { 
            // out of memory
            GB_FREE_ALL ;
    ...
    if (P != NULL)
    {
        ...
        else
        {
            ...
            // is copied to Ph.
            P->plen = cnvec ;
            P->x = GB_MALLOC (int64_t, cnz, P->type, &(P->x_size)) ; // x:OK
            P->p = GB_MALLOC (int64_t, cnvec+1, GrB_INT64, &(P->p_size)) ;
            P->h = NULL ;
            if (C_is_hyper)
            { 
                P->h = GB_MALLOC (int64_t, cnvec, GrB_INT64, &(P->h_size)) ;
            }
